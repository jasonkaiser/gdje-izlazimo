...
export class AdminVenuesComponent implements OnInit {
  ...
  private initializeStreams(): void {
    this.venues$ = this.refresh$.pipe(
      switchMap(() =>
        this.venueService.getVenues({ pageSize: 1000, sortBy: 'id', sortDir: 'DESC' }).pipe(
          catchError((err) => {
  ...
  private applyStatusFilter(venues: VenueResponseDto[], filter: StatusFilter): VenueResponseDto[] {
    ...
    return venues.filter((v) => (filter === 'ACTIVE' ? v.isActive : !v.isActive));
  }
  ...
  openCreateVenueModal(): void {
    this.modalService.open(VenueModalComponent, { data: { mode: 'create' } });
  }
  ...
  toggleVenueStatus(venue: VenueResponseDto): void {
    ...
    this.venueService.updateVenue(updateRequest, venue.id)
      ...
      .subscribe({
        next: () => {
          this.refresh$.next();
          this.toastService.show(`Lokal ${newStatus ? 'aktiviran' : 'deaktiviran'}`, 'success');
        },
        error: () => {
